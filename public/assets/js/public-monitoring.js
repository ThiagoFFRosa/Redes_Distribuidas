document.addEventListener('DOMContentLoaded', () => {
  ...
  const loadHistoricalChart = async (point) => {
    ...
    const isReady = ['READY', 'STALE'].includes(payload.status) && normalizedPayload?.labels?.length && normalizedPayload?.datasets?.[0]?.data?.length;
    if (!isReady) { empty.textContent = payload.message || (payload.status === 'NO_DATA' ? 'Este ponto ainda não possui dados históricos ou medições cadastradas.' : 'Gráfico histórico ainda está sendo gerado.'); empty?.classList.remove('hidden'); canvas?.classList.add('hidden'); return; }
    empty?.classList.add('hidden'); canvas?.classList.remove('hidden');
    const points = normalizedPayload.points || [];
    state.chart = new Chart(canvas, { type: 'line', data: { labels: normalizedPayload.labels, datasets: normalizedPayload.datasets.map((dataset) => ({ ...dataset, borderColor: dataset.borderColor || '#0d9488', backgroundColor: dataset.backgroundColor || 'rgba(13,148,136,0.10)', fill: dataset.fill ?? true, tension: dataset.tension ?? 0.2, pointRadius: dataset.pointRadius ?? 0 })) }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: true }, subtitle: { display: Boolean(payload.message), text: payload.message || '' }, tooltip: { callbacks: { label: (context) => { const pointInfo = points[context.dataIndex] || {}; const source = pointInfo.source === 'SITE' ? 'Site' : (pointInfo.source || 'CSV'); return `${context.label} — ${context.parsed?.y ?? context.raw} ${pointInfo.unit || normalizedPayload.unit || 'm'} — Origem: ${source}`; } } } }, scales: { y: { title: { display: true, text: normalizedPayload.unit || 'm' } } } } });
  };
