...
const upsertHistoricalMeasurement = async (payload, connection) => {
  await connection.execute(
    ...
    [payload.data_point_id, payload.import_id ?? null, toMysqlDate(payload.measured_at), payload.raw_value ?? null, payload.raw_unit || 'cm', payload.value, payload.unit || 'm', payload.max_value ?? null, payload.min_value ?? null, payload.source || 'CSV_IMPORT']
  );
};
